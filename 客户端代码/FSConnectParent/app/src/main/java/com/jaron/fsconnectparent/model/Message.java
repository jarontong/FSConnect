...
import com.jaron.fsconnectparent.utils.TimeUtil;
    ...
    public RelativeLayout getBubbleView(ChatAdapter.ViewHolder viewHolder,Context context){
        ...
        if (message.isSelf()){
            ...
            if(message.getSenderProfile()!=null) {
                String url = message.getSenderProfile().getFaceUrl();
                ImageLoader.loadImage(Glide.with(context),viewHolder.rightAvatar, message.getSenderProfile().getFaceUrl(), R.drawable.head_other);
            }
            ...
            String url=message.getSenderProfile().getFaceUrl();
            ImageLoader.loadImage(Glide.with(context),viewHolder.leftAvatar, message.getSenderProfile().getFaceUrl(), R.drawable.head_other);
            ...
            if (message.getConversation().getType() == TIMConversationType.Group){
                ...
                if (name.equals("")) name = message.getSender().substring(message.getSender().length()-11);
