...
import com.jaron.fsconnectparent.adapter.ProfileSummaryAdapter;
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_ENTER:
                Log.d("id", TIMFriendshipManager.getInstance().getIdentifier());
                list.clear();
                adapter.notifyDataSetChanged();
                String key = mSearchInput.getText().toString();
                if (key.equals("")) return true;
                presenter.searchFriendById("86_"+key);
                //presenter.searchFriendByName("86_"+key,true);
                return true;
            default:
