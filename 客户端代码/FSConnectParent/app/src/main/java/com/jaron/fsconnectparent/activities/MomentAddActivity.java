...
import com.jaron.fsconnectparent.adapter.GridImageAdapter;
...
//                    selectList = PictureSelector.obtainMultipleResult(data);
    ...
    private void addMoment(){
        ...
        Integer parentId= AccountHelper.getUserId();
        int size=selectList.size();
        File[] imageFiles=new File[size];
        for(int i=0;i<size;i++){
            imageFiles[i]=new File(getImagePath(selectList.get(i)));
        }
        FSConnectApi.addMoment(null,parentId,momentContent,imageFiles,new TextHttpResponseHandler() {
